...
public class AccountResource {
    ...
    public ResponseEntity<?> registerAccount(@Valid @RequestBody ManagedUserVM managedUserVM, HttpServletRequest request) {
        ...
        return userRepository.findOneByLogin(managedUserVM.getEmail().toLowerCase())
        
            .map(user -> new ResponseEntity<>("login already in use", textPlainHeaders, HttpStatus.BAD_REQUEST))
            .orElseGet(() -> userRepository.findOneByEmail(managedUserVM.getEmail())
                .map(user -> new ResponseEntity<>("e-mail address already in use", textPlainHeaders, HttpStatus.BAD_REQUEST))
                .orElseGet(() -> {
                    User user = userService.createUser(managedUserVM.getEmail(), managedUserVM.getPassword(),
                    managedUserVM.getEmail(), managedUserVM.getLastName(), managedUserVM.getEmail().toLowerCase(),
